...
interface GeneralChatProps {
  printerFrame: SerialFrame | null
}

const SUGGESTIONS = [
  'Best PLA print temp for MK3S+?',
  'How do I calibrate first layer?',
  'Why is my print stringing?',
  'How often should I lubricate rods?',
]

export function GeneralChat({ printerFrame }: GeneralChatProps) {
  ...
  async function send(text?: string) {
    ...
    try {
      const res = await fetch('/api/chat/general', {
        ...
        body: JSON.stringify({ messages: nextMessages, printerFrame }),
      })
  ...
  return (
    <div className="wd-chat">
      <div className="wd-chat__head">
        <span className="wd-chat__title">
          <Bot size={15} />
          Assistant
          {printerFrame && (
            <span className="wd-badge wd-badge--live" style={{ marginLeft: 4 }}>
              <span className="wd-dot" />context
            </span>
          )}
        </span>
        {messages.length > 0 && (
          <button className="wd-chat__clear" onClick={() => setMessages([])}>Clear</button>
        )}
      </div>
      <div className="wd-chat__body">
        {messages.length === 0 ? (
          <div>
            <p className="wd-chat__hint">Ask anything about your Prusa MK3S+</p>
      ...
      <div className="wd-chat__input">
        <input
          ...
          placeholder="Ask about printing, settings, troubleshooting…"
          disabled={streaming}
        />
